...
function playAudio(url, title) {
    ...
    audio.play().then(() => {
        ...
        alert("عذراً، هذا البث غير متاح حالياً.");
    });
}
    ...
    try {
        ...
        let html = `
            <div class="sticky-nav">
                ...
                <span class="font-bold text-xs">قائمة القراء</span>
            </div>`;
        html += data.reciters.slice(0, 50).map(r => `
            <div class="card mb-3 flex justify-between items-center px-4" onclick="openSurahs('${r.moshaf[0].server}', '${r.name}', '${r.moshaf[0].surah_list}')">
                <span class="text-xs font-bold">${r.name}</span>
                <i class="fas fa-chevron-left text-gray-400 text-xs"></i>
            </div>
        `).join('');
        showPage(html);
    } catch (e) { showPage("<p>خطأ في الاتصال</p>"); }
}
...
function loadAzkar(t) {
    const azkar = t === 'morning' ? ["آية الكرسي", "أصبحنا وأصبح الملك لله", "اللهم بك أصبحنا"] : ["آية الكرسي", "أمسينا وأمسى الملك لله", "اللهم بك أمسينا"];
    ...
    html += azkar.map(z => `<div class="card mb-2 text-right text-xs p-4">${z}</div>`).join('');
    document.getElementById('content-area').innerHTML = html;
}

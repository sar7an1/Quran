...
const azkarData = {
    morning: [
        ...
        {text:"رضيت بالله رباً وبالإسلام ديناً وبمحمد ﷺ نبياً (3)", count:3},
...
async function openReciters() {
    ...
    const data = await res.json();
    let html = `<div class="sticky-nav"><button onclick="goHome()" class="back-btn">رجوع</button><span>أصوات مختلفة</span></div>`;
...
function openRoqia() {
    ...
    <div class="menu-grid">
        <div class="card" onclick="playAudio('https://server12.mp3quran.net/maher/115.mp3', 'رقية ماهر المعيقلي')"><i class="fas fa-heart-pulse" style="color:red"></i><span>ماهر المعيقلي</span></div>
        <div class="card" onclick="playAudio('https://server8.mp3quran.net/afs/115.mp3', 'رقية مشاري العفاسي')"><i class="fas fa-heart-pulse" style="color:red"></i><span>مشاري العفاسي</span></div>
    </div>`;
    showPage(html);
}
